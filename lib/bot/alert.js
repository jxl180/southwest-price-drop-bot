...
class Alert {
  ...
  constructor (data) {
    ...
    if (this.data.phone != null) {this.data.phone = this.data.phone.split('').filter(d => /\d/.test(d)).join('');}
    if (this.data.to_email != null) {this.data.to_email = this.data.to_email.split('').filter(d => /\S/.test(d)).join('');}
  ...
  get initialCheck () { return this.data.initialCheck; }
  ...
  async getLatestPrice () {
    ...
    if (this.initialCheck === false) this.data.initialCheck = true;
    return price;
  }
